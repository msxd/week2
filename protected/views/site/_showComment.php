...
<div class="col-xs-8 col-xs-offset-2" id="comments">
    ...
    foreach ($comments as $comment) {
		...
		<blockquote>
			<p>' . $comment->body . '</p>
			<div style="text-align:right"><button type="button" class="btn btn-xs" onclick="scroll_to_elem(\'addComment\',2000,' . $comment->id . ')">Ответить</button>' . ((Yii::app()->user->checkAccess(User::ROLE_MODER)) ? '<button type="button" class="btn btn-xs removeComment" style="margin-left: 10px;" data-url="' . Yii::app()->createUrl('site/removecomment/' . $comment->id) . '"><span class="glyphicon glyphicon-trash"></span></button>' : '') . '</div>
